...
func _ready():
    ...
    $ThinkTimer.connect("timeout", self, "_on_think_timer_timeout")

func initialize(state_ref, bot_color, bot_difficulty):
...
func _on_think_timer_timeout():
    ...
    if not success:
        ...
        if game_state.is_checkmate(player_color):
            get_parent().emit_signal("game_over", Globals.Player.WHITE if player_color == Globals.Player.BLACK else Globals.Player.BLACK)
        elif game_state.is_stalemate(player_color):
            get_parent().emit_signal("game_over", null)  # 引き分け

func _make_random_move():
    ...
    if not captured_pieces.empty() and randf() < 0.3:
...
func _make_medium_move():
    ...
    if not check_moves.empty():
        check_moves.shuffle()
        selected_move = check_moves[0]
    elif not capture_moves.empty():
        # 価値の高い駒を優先して取る
        capture_moves.sort_custom(self, "_sort_by_capture_value")
        selected_move = capture_moves[0]
    elif not safe_moves.empty():
        safe_moves.shuffle()
...
func _make_hard_move():
    # 最も強い動き（将来的にミニマックスアルゴリズムなどで実装）
    # 現時点ではミディアムと同じ
    return _make_medium_move()

func _place_random_captured_piece():
    var captured_pieces = game_state.captured_pieces[player_color]
...
func _place_strategic_captured_piece():
    var captured_pieces = game_state.captured_pieces[player_color]
    if captured_pieces.empty():
        return false
...
func _move_causes_check(piece, target_pos, opponent):
    var original_pos = piece.board_position
    var original_target_piece = game_state.board[target_pos.x][target_pos.y]
...
func _is_threatened_after_move(piece, target_pos):
    var original_pos = piece.board_position
    var original_target_piece = game_state.board[target_pos.x][target_pos.y]
...
func _is_threatened_position(pos):
    var opponent = Globals.Player.BLACK if player_color == Globals.Player.WHITE else Globals.Player.WHITE
    
    # その位置が相手の駒に攻撃されるかチェック
    return game_state._is_square_attacked(pos, player_color)

func _placement_causes_check(piece, pos, opponent):
    # 一時的に配置
...
func _placement_causes_fork(piece, pos):
    var opponent = Globals.Player.BLACK if player_color == Globals.Player.WHITE else Globals.Player.WHITE
...
func _can_attack_from_to(piece, from_pos, to_pos):
    # 駒の種類に応じて攻撃可能かチェック
    match piece.type:
...
func _sort_by_capture_value(a, b):
    # 駒の価値
    var piece_values = {
