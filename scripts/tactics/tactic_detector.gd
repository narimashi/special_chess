...
func detect_tactics(piece, from_pos: Vector2, to_pos: Vector2) -> Array:
	var tactics = []
...
func _is_castling(piece, from_pos: Vector2, to_pos: Vector2) -> bool:
...
func _is_fork(piece, to_pos: Vector2) -> bool:
	var opponent = Globals.Player.BLACK if piece.player == Globals.Player.WHITE else Globals.Player.WHITE
...
func _is_pin(piece, to_pos: Vector2) -> bool:
	var opponent = Globals.Player.BLACK if piece.player == Globals.Player.WHITE else Globals.Player.WHITE
	...
	var opponent_king = game_state._find_king(opponent)
	...
	return not pinned_pieces.is_empty()

func _is_discovered_attack(piece, from_pos: Vector2, to_pos: Vector2) -> bool:
	var opponent = Globals.Player.BLACK if piece.player == Globals.Player.WHITE else Globals.Player.WHITE
	...
	return not attacking_pieces.is_empty()

func _causes_check(piece, to_pos: Vector2, opponent: int) -> bool:
	var original_piece = game_state.board[to_pos.x][to_pos.y]
	var original_pos = piece.board_position
...
func _can_attack(piece, target_pos: Vector2) -> bool:
	var original_pos = piece.board_position
...
func _can_attack_in_direction(piece, direction: Vector2) -> bool:
	match piece.type:
		Globals.PieceType.ROOK:
			# ルークは縦横方向のみ
