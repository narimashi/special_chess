...
class ChessPiece:
	...
	func _init(piece_type: int, piece_player: int, pos: Vector2):
		type = piece_type
		player = piece_player
		board_position = pos
...
func _add_piece(type: int, player: int, pos: Vector2):
...
func get_captured_pieces() -> Dictionary:
	return captured_pieces

func move_piece(piece, to_pos: Vector2, promotion_type = null) -> bool:
...
func undo_move() -> bool:
	...
	if move.has("is_drop") and move["is_drop"]:
		...
		if move["captured"]:
			...
			if index >= 0:
				captured_pieces[piece.player].remove_at(index)
...
func _has_moved_before(piece, pos: Vector2) -> bool:
...
func _get_pawn_moves(piece) -> Array:
	var moves = []
	var pos = piece.board_position
...
func _get_knight_moves(piece) -> Array:
	var moves = []
	var pos = piece.board_position
	var offsets = [
...
func _get_sliding_moves(piece, directions: Array) -> Array:
	var moves = []
	var pos = piece.board_position
...
func _get_king_moves(piece) -> Array:
	var moves = []
	var pos = piece.board_position
...
func _filter_moves_that_prevent_check(piece, moves: Array) -> Array:
	var valid_moves = []
...
func _is_square_attacked(pos: Vector2, defender_player: int) -> bool:
